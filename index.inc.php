...
<div class="content">
  ...
  <h1 iclass="content__heading"><font color=white>Napster Proxy</font></h1>
...
?>
  <form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
    <ul id="form">
       <input id="address_box" type="text" name="<?php echo $GLOBALS['_config']['url_var_name'] ?>" placeholder="Enter your URL here" value="<?php echo isset($GLOBALS['_url']) ? htmlspecialchars($GLOBALS['_url']) : '' ?>" onfocus="this.select()" /></label> <input id="go" type="submit" value="Go" /></li>
      ...
      foreach ($GLOBALS['_flags'] as $flag_name => $flag_value)
      {
          ...
          {
              echo '<li><font color=white><label><input type="checkbox" name="' . $GLOBALS['_config']['flags_var_name'] . '[' . $flag_name . ']"' . ($flag_value ? ' checked="checked"' : '') . ' />' . $GLOBALS['_labels'][$flag_name][1] . '</label></li>' . "\n";
          }
      }
      ?>
